...
export async function GET(
  ...
  const { data: change, error } = await (db as any)
    .from("content_change_requests")
    .select("requester_id, proposed_values")
    .eq("id", id)
    .maybeSingle();
  if (error) return Response.json({ error: error.message }, { status: 500 });
  if (!change || (change.requester_id !== user.id && !isAdmin))
    return Response.json({ error: "forbidden" }, { status: 403 });
  if (!isAdmin && !path.startsWith(`${user.id}/`))
    return Response.json({ error: "invalid_path" }, { status: 400 });
  if (!containsPath(change.proposed_values, path))
    return Response.json({ error: "path_not_in_request" }, { status: 404 });
  const { data: signed, error: signedError } = await db.storage
    .from("content-change-media")
    .createSignedUrl(path, 60);
  if (signedError || !signed)
    return Response.json(
      { error: signedError?.message ?? "preview_unavailable" },
      { status: 500 },
    );
