...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  if (!data) return { title: "კვება | MyBakuriani" };

  return {
    title: `${data.title} | კვება | MyBakuriani`,
    description:
      data.description ?? `${data.cuisine_type ?? ""} რესტორანი ბაკურიანში`,
  };
}
...
function parseMenu(menu: Json | null): MenuItem[] {
  if (!menu || !Array.isArray(menu)) return [];
  return (menu as unknown[]).filter(
...
export default async function FoodDetailPage({ params }: Props) {
  ...
  const menuItems = parseMenu(service.menu);
        ...
        <div className="lg:col-span-2">
          <div className="mb-4 flex flex-wrap items-center gap-3">
            ...
            {service.cuisine_type && (
              <Badge variant="secondary">{service.cuisine_type}</Badge>
            )}
            {service.has_delivery && <Badge variant="outline">🚚 მიტანა</Badge>}
            {service.is_vip && (
              ...
              <div className="divide-y divide-border rounded-xl border border-border">
                {menuItems.map((item, i) => (
                  <div key={i} className="flex items-start justify-between p-4">
                    ...
                    <p className="ml-4 shrink-0 font-bold text-foreground">
