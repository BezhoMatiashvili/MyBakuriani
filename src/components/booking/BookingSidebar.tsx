...
import { formatPrice, formatPhone, maskPhone } from "@/lib/utils/format";
import {
  differenceInDays,
  format,
  addMonths,
        ...
        <div className="mt-5 flex gap-2">
          {ownerPhone ? (
            <a
              ...
              onClick={(e) => {
                ...
                if (isDesktop && !phoneRevealed) {
                  e.preventDefault();
                  setPhoneRevealed(true);
                }
              }}
              className="inline-flex h-12 flex-1 items-center justify-center gap-2 rounded-full bg-[#F97316] text-[14px] font-bold text-white shadow-[0px_8px_20px_rgba(249,115,22,0.25)] transition-colors hover:bg-[#EA580C]"
            >
              <Phone className="h-4 w-4" />
              <span className="md:hidden">დარეკვა მესაკუთრეთან</span>
          ...
          {ownerPhone ? (
            <a
              ...
              rel="noreferrer"
              aria-label="WhatsApp"
              className="flex h-12 w-12 shrink-0 items-center justify-center rounded-full bg-[#25D366] text-white transition-colors hover:bg-[#25D366]/90"
            >
